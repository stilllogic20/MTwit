...
from mTwit.notifications import observe, NotificationKind


class NotificationWindow(QDialog):  # ErrorWindowと統合してもいいかもしれない

    @classmethod
    def display_notifications(cls, kind, details):
        NotificationWindow(time=2000, message=details).show(kind)
...
@observe
def _observer(kind, details):
    NotificationWindow.display_notifications(kind, details)
